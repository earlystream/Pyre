...
import pyre.common.compat.ServerTickCompat;
import pyre.common.config.PyreServerConfig;
...
public final class PyreExplosionManager {
    ...
    private PyreServerConfig config = new PyreServerConfig();
    ...
    public void configure(PyreServerConfig config, CompatFlags compatFlags) {
        this.config = config;
    ...
    public void onWorldTickStart(ServerWorld world) {
        ...
        this.stateFor(world).beginTick(ServerTickCompat.currentTick(world));
    }
    ...
    public void onExplosionStart(Explosion explosion) {
        ...
        ServerWorld world = ExplosionCompat.world(explosion);
        WorldState state = this.stateFor(world);
        long tick = ServerTickCompat.currentTick(world);
        ...
        ExplosionClusterIndex.ClusterMembership membership = this.config.enableClusterReuse
                ? state.clusterIndex.register(tick, ExplosionCompat.position(explosion), ExplosionCompat.power(explosion), this.config.maxClusterRadius)
                : ExplosionClusterIndex.isolated(tick);
    ...
    public void onExplosionEnd(Explosion explosion) {
        if (!this.config.enabled) {
            return;
        }

        WorldState state = this.worldStates.get(ExplosionCompat.world(explosion).getRegistryKey());
        if (state == null) {
            return;
        }
    ...
    public List<Entity> queryNearbyEntities(ServerWorld world, Explosion explosion, @Nullable Entity except, Box box, Supplier<List<Entity>> vanillaQuery) {
        ...
        List<Entity> cached = state.queryCache.get(tick, except, box).orElse(null);
        ...
        state.queryCache.put(tick, except, box, result);
    ...
    private static final class WorldState {
        ...
        private void finishTick(ServerWorld world, PyreServerConfig config) {
